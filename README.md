...
An iOS pocket browser is in progress, on top of the same `LimeghostCore`. As of September 8, 2026 there is an app: `ios/Limeghost.xcodeproj` builds a phone browser with tabs, a bottom bar, an address sheet with local-only completion, a tab switcher with private tabs, and the AI guide as its start surface — all driving the Mac's own `BrowserWorkspace` through the shared `LimeghostShared` target, not a phone-shaped copy of it. It runs on a Simulator with 22 tests passing and it builds for a device, but **nothing has been installed on a phone yet**, and the AI guide does not fit a phone screen — a named blocking release gate, not a polish note. There is no assistant, no Reader, no Copy for AI and no bookmark import in the phone app. See [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md) for what exists and [docs/superpowers/specs/2026-09-03-ios-pocket-browser-design.md](docs/superpowers/specs/2026-09-03-ios-pocket-browser-design.md) for the full design.
...
- The iOS app runs on a Simulator and has never been on a phone. It has tabs, a bottom bar, an address sheet with local-only completion, a tab switcher with private tabs, and the AI guide; it has no assistant, no Reader, no Copy for AI, no bookmark import, no downloads and no settings screen. Signing is set to automatic with a deliberately empty team, so a device build is one Apple ID away — but free provisioning issues a **seven-day** certificate, which means the app stops launching after a week until it is run from Xcode again, and it reaches only the devices attached to that one Apple ID. Nobody but the founder can install it until Apple Developer Program enrolment. Nothing is signed for distribution, notarized, on TestFlight, or App Store ready, and **no observed-user session has been run for this project on either platform**. Two blocking release gates are recorded in [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md), which also carries the device procedure: the AI guide, the app's start surface, does not fit a phone screen — at 402pt the headline runs eight lines past the bottom of the display and nothing actionable is above the fold — and the address sheet completes a private tab from the ordinary profile's history, which the Mac refuses to do. The CI job that builds and tests it has itself never executed, because CI triggers only on push to `main` and on pull requests.
