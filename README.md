...
- The folder icon picker offers four sets. The Clearframe set (104 icons) is the project's own artwork. The rest are licensed and credited in the picker wherever they are shown. Whether a folder's tint reaches a set depends on the artwork: Clearframe and Elegant are drawn in `currentColor` and take the four tints; Stickies and Emoji carry their own colours, so the swatch row hides rather than showing a control that does nothing.
  - **Elegant** (100 drawings) — ["Elegant" by Kenny Sing](https://github.com/kennysing/Elegant-Icons), [GPL-3.0](https://www.gnu.org/licenses/gpl-3.0.html). No attribution required; credited anyway. **This one is copyleft over the whole program, not just over adaptations of the artwork.** GPL-3.0 is compatible with this repository's AGPL-3.0 and the combined work stays AGPL-3.0, so shipping it as the project stands today is fine — but it would have to be removed, or separately licensed from its author, before Clearframe could be offered under closed commercial terms. Iconify summarises the licence as "commercial use is allowed", which is true and easy to misread: GPL software may be sold, but it must still be conveyed under the GPL with source.
