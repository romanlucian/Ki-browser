...
- An iOS app does not exist yet. `LimeghostShared`, the platform-neutral layer it will be built on, does: it typechecks against the iOS 17 SDK, and on September 3, 2026 its tests were run on an iPhone 17 Pro simulator and passed, including the rule that every way of asking for a page uncovers the page. But there is no `ios/` Xcode project, no phone UI, no App Store presence, and nothing installed on a real device. The CI job that runs those tests has itself never executed, because CI triggers only on push to `main` and on pull requests. See [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md).
