...
An iOS pocket browser is in progress, on top of the same `LimeghostCore`. As of September 10, 2026 there is an app: `ios/Limeghost.xcodeproj` builds a phone browser with tabs, a bottom bar, an address sheet with local-only completion, a tab switcher with private tabs, and the AI guide as its start surface — all driving the Mac's own `BrowserWorkspace` through the shared `LimeghostShared` target, not a phone-shaped copy of it. It runs on a Simulator with 28 tests passing, and compiles for the device SDK, though a device build stops at signing until an Apple ID team is selected. It was first installed on the founder's own iPhone on September 10, 2026, under free provisioning, which reaches no other phone and lasts seven days. The AI guide did not fit a phone screen until September 10, 2026; its header now stacks wherever the Mac's one row does not fit. There is no assistant, no Reader, no Copy for AI and no bookmark import in the phone app. See [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md) for what exists and [docs/superpowers/specs/2026-09-03-ios-pocket-browser-design.md](docs/superpowers/specs/2026-09-03-ios-pocket-browser-design.md) for the full design.
...
- a seven-step first-run introduction covering Limeghost's promise, search choice, the privacy boundary, the assistant beside the page, Compare answers, Reader, and bookmark folder icons. Its feature counts are read from the catalogues at runtime rather than typed in, and a test forbids it from ever again describing an online provider Limeghost does not have;
...
- The iOS app runs on a Simulator and has never been on a phone. It has tabs, a bottom bar, an address sheet with local-only completion, a tab switcher with private tabs, and the AI guide; it has no assistant, no Reader, no Copy for AI, no bookmark import, no downloads and no settings screen. Signing is set to automatic with a deliberately empty team, so a device build is one Apple ID away — but free provisioning issues a **seven-day** certificate, which means the app stops launching after a week until it is run from Xcode again, and it reaches only the devices attached to that one Apple ID. Nobody but the founder can install it until Apple Developer Program enrolment. Nothing is signed for distribution, notarized, on TestFlight, or App Store ready, and **no observed-user session has been run for this project on either platform**. The AI guide, the app's start surface, does not fit a phone screen: at 402pt the headline runs eight lines past the bottom of the display and nothing actionable is above the fold. That is recorded as a blocking release gate in [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md), which also carries the device procedure. The CI job that builds and tests it has itself never executed, because CI triggers only on push to `main` and on pull requests.
