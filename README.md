...
An iOS pocket browser is in progress, on top of the same `LimeghostCore`. As of September 10, 2026 there is an app: `ios/Limeghost.xcodeproj` builds a phone browser with tabs, a bottom bar, an address sheet with local-only completion, a tab switcher with private tabs, and the AI guide as its start surface — all driving the Mac's own `BrowserWorkspace` through the shared `LimeghostShared` target, not a phone-shaped copy of it. It runs on a Simulator with 27 tests passing, and compiles for the device SDK, though a device build stops at signing until an Apple ID team is selected. It was first installed on the founder's own iPhone on September 10, 2026, under free provisioning, which reaches no other phone and lasts seven days. The AI guide does not fit a phone screen — a named blocking release gate, not a polish note. There is no assistant, no Reader, no Copy for AI and no bookmark import in the phone app. See [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md) for what exists and [docs/superpowers/specs/2026-09-03-ios-pocket-browser-design.md](docs/superpowers/specs/2026-09-03-ios-pocket-browser-design.md) for the full design.
