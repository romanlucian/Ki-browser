...
///
/// WebKit applies the compiled rules inside the page process, so Limeghost can
...
final class ContentRuleListProvider: ObservableObject {
    ...
    private func performApply() async {
        ...
        guard let ruleStore else {
            status = .unavailable("Limeghost could not open the local filter store.")
            return
        }
        ...
        do {
            ...
            appliedIdentifier = nil
            attachToRegisteredWebViews(nil)
            status = .unavailable("Limeghost could not load the tracker filter.")
        }
    }
