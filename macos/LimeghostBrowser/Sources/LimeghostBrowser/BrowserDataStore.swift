...
final class BrowserDataStore: ObservableObject {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        if !unreadableNames.isEmpty {
            recoveryNotice = "Limeghost could not read saved \(unreadableNames.joined(separator: ", ")). The original data was preserved for recovery and was not replaced."
        } else if !recoveredNames.isEmpty {
            recoveryNotice = "Limeghost restored \(recoveredNames.joined(separator: ", ")) from the last known-good local backup."
        } else {
            recoveryNotice = nil
        }
    ...
    func loadWorkspace() -> BrowserWorkspaceSnapshot? {
        ...
        if load.wasRecovered {
            recoveryNotice = "Limeghost restored the previous tab session from the last known-good local backup."
        } else if load.isUnreadable {
            recoveryNotice = "Limeghost could not read the previous tab session. The original data was preserved and no unsafe URL was restored."
        }
