...
struct BookmarkOrganizerView: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: 8) {
            ...
            .font(.caption2)
            .foregroundStyle(dropConfirmation == nil ? Color.secondary : LimeghostTheme.accent)
...
struct BookmarkFolderRow: View {
    ...
    var body: some View {
        HStack(spacing: 9) {
            Button(action: open) {
                HStack(spacing: 9) {
                    BookmarkFolderIcon(folder: folder, size: 17).foregroundStyle(LimeghostTheme.textSecondary)
        ...
        .padding(9)
        .background(
            isDropTargeted ? LimeghostTheme.accentDim : Color.primary.opacity(0.045),
            in: RoundedRectangle(cornerRadius: 9)
        )
        .overlay {
            RoundedRectangle(cornerRadius: 9)
                .stroke(isDropTargeted ? LimeghostTheme.accent : Color.clear, lineWidth: 1.5)
        }
...
struct BookmarkFolderEditor: View {
    ...
    init(request: BookmarkFolderEditorRequest, save: @escaping (String, String, String) -> Void) {
        ...
        _color = State(initialValue: LimeghostIconColor(id: request.colorID) ?? .mint)
    ...
    /// Limeghost icon still selected must not hide the tint that icon is
    ...
    private var iconPreview: some View {
        let icon = LimeghostIconView(iconID: iconID, size: 24)
        Group {
            if selectedIsTintable {
            ...
            ForEach(LimeghostIconColor.allCases, id: \.self) { swatch in
                Button { color = swatch } label: {
                    Circle()
                        ...
                        .overlay {
                            Circle()
                                .stroke(LimeghostTheme.textPrimary, lineWidth: swatch == color ? 2 : 0)
                        }
                }
                .buttonStyle(.plain)
                .help(swatch.title)
