...
/// What importing one parsed `BookmarkImport` into Limeghost's own
...
public enum BookmarkImportPlacement: String, Equatable, Sendable, CaseIterable {
    /// The source's own bar becomes Limeghost's bar. Nothing wraps the
...
public struct BookmarkImportPlan: Equatable, Sendable {
    ...
    /// includes a folder whose entire subtree ended up empty — Limeghost
    ...
    /// is kept, and Limeghost never merges an import into a folder somebody
