...
/// One icon in the shipped Limeghost set: a stable identifier, the group the
/// picker files it under, and the artwork itself.
///
/// `markup` is the drawing, held exactly as it was drawn — a 16x16 box of
/// stroke-only geometry. It is deliberately not pre-parsed here: `LimeghostCore`
/// stays Foundation-only, and the UI layer turns markup into a drawable path
/// once per icon through `VectorPathParser`.
public struct LimeghostIcon: Equatable, Identifiable, Sendable {
    public let id: String
    public let category: LimeghostIconCategory
    public let markup: String
    /// Which shipped set this icon belongs to. Drives how it is drawn and
    /// whether a folder tint can reach it.
    public let style: LimeghostIconStyle
    /// The coordinate box the artwork was drawn in. The Limeghost set is a
    ...
    public init(
        id: String,
        category: LimeghostIconCategory,
        markup: String,
        style: LimeghostIconStyle = .limeghost,
...
/// value type so `LimeghostCore` stays Foundation-only.
...
/// Limeghost's own set is drawn in `currentColor`, so a folder's tint reaches
/// it; the licensed sets carry their own colours, so a tint has nothing to act
/// on and the swatch row is hidden rather than shown doing nothing. That is a
/// property of the artwork, not a gap in the picker.
///
/// Limeghost's own set stays the default: it is the only one that includes the
/// plain folder every unset folder falls back to.
///
/// A fourth set, Elegant, was shipped briefly and withdrawn. It was tintable
/// and good, but GPL-3.0: copyleft over the whole program rather than only over
/// adaptations of the artwork, which would have removed the maintainer's option
/// to offer separate commercial terms. Weigh that before adding another set.
public enum LimeghostIconStyle: String, CaseIterable, Sendable {
    case limeghost
    case stickies
    case emojiOne
    ...
    public var title: String {
        switch self {
        case .limeghost: return "Limeghost"
    ...
    ///
    /// The Limeghost set is drawn to be rounded throughout, so that is its
    ...
    ///
    /// True for any set drawn in `currentColor`, which today is only
    /// Limeghost's own. It is a switch rather than an equality check because
    /// the answer belongs to the artwork: a set that names no colour is
    /// tintable whoever drew it.
    public var isTintable: Bool {
        switch self {
        case .limeghost: return true
...
public enum LimeghostIconCategory: String, CaseIterable, Sendable {
    case work
    case creative
    case reading
    case shopping
...
/// The folder icons Limeghost ships, and the rules for choosing one.
///
...
public enum LimeghostIconCatalog {
    ...
    /// Every icon the app ships, in catalog order: Limeghost's own set first,
    ...
    public static let all: [LimeghostIcon] =
        LimeghostIconCatalogData.icons
        + StickiesIconCatalogData.icons
        + EmojiOneIconCatalogData.icons

    private static let index: [String: LimeghostIcon] = Dictionary(
        uniqueKeysWithValues: all.map { ($0.id, $0) }
    )

    private static let byStyle: [LimeghostIconStyle: [LimeghostIcon]] =
        Dictionary(grouping: all, by: \.style)

    private static let byStyleAndCategory: [LimeghostIconStyle: [LimeghostIconCategory: [LimeghostIcon]]] =
    ...
    public static func icon(id: String) -> LimeghostIcon? {
        index[id]
    }

    /// One style's icons, in catalog order.
    public static func icons(style: LimeghostIconStyle) -> [LimeghostIcon] {
        byStyle[style] ?? []
    }

    /// One style's icons grouped for the picker.
    public static func byCategory(style: LimeghostIconStyle) -> [LimeghostIconCategory: [LimeghostIcon]] {
        byStyleAndCategory[style] ?? [:]
    }

    /// Defaults to Limeghost's own set, which is what every surface outside
    /// the picker means when it asks for a category.
    public static func icons(
        in category: LimeghostIconCategory,
        style: LimeghostIconStyle = .limeghost
    ) -> [LimeghostIcon] {
    ...
    /// empty tab.
    public static var availableStyles: [LimeghostIconStyle] {
        LimeghostIconStyle.allCases.filter { !icons(style: $0).isEmpty }
    }
...
public enum LimeghostIconColor: String, CaseIterable, Sendable {
    case mint
    case grey
    case amber
    case blue
    ...
    public init?(id: String?) {
        guard let id, let color = LimeghostIconColor(rawValue: id.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()) else {
            return nil
        }
        self = color
    }
    ...
    public static func normalizedID(_ value: String?) -> String? {
        LimeghostIconColor(id: value)?.rawValue
    }
}
