...
public enum WebURLPolicy {
    /// Returns a normalized URL that is safe to navigate to or persist. Limeghost
...
public struct BookmarkFolderRecord: Codable, Equatable, Identifiable, Sendable {
    public let id: UUID
    public var title: String
    /// The emoji this folder was saved with, before Limeghost had drawn icons.
    ///
    ...
    /// The chosen Limeghost icon, or `nil` for a folder that predates the set
    ...
    public init(from decoder: Decoder) throws {
        ...
        colorID = LimeghostIconColor.normalizedID(try container.decodeIfPresent(String.self, forKey: .colorID))
    ...
    public var resolvedIconID: String {
        LimeghostIconCatalog.resolvedIconID(iconID: iconID, legacyEmoji: emoji)
    }

    /// The tint this folder's icon draws in, falling back to the set's default.
    public var resolvedColor: LimeghostIconColor {
        LimeghostIconColor(id: colorID) ?? .mint
    }
