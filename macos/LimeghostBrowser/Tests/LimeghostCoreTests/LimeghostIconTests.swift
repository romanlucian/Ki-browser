...
final class LimeghostIconTests: XCTestCase {

    // MARK: - The catalog

    func testCatalogShipsOneHundredFourIconsAcrossThirteenCategories() {
        XCTAssertEqual(LimeghostIconCatalog.icons(style: .limeghost).count, 104)
        XCTAssertEqual(LimeghostIconCategory.allCases.count, 18)

        let expectedCounts: [LimeghostIconCategory: Int] = [
        ...
        for category in LimeghostIconCategory.allCases {
            XCTAssertEqual(
                LimeghostIconCatalog.icons(in: category).count,
        ...
        XCTAssertEqual(
            ...
            LimeghostIconCatalog.icons(style: .limeghost).count,
    ...
    func testStickiesShipsOnlyTheFormThatDoesNotFightTheAccent() {
        let stickies = LimeghostIconCatalog.icons(style: .stickies)
        ...
        XCTAssertEqual(
            LimeghostIconCatalog.all.count,
            104 + 100 + 1261,
            "one lookup has to answer for every style"
        )
        XCTAssertEqual(LimeghostIconCatalog.availableStyles, LimeghostIconStyle.allCases)
    }
    ...
    func testAFolderSavedWithTheWithdrawnFormKeepsItsDrawing() {
        XCTAssertEqual(
            LimeghostIconCatalog.resolvedIconID(iconID: "stickies-duo-mail", legacyEmoji: ""),
            "stickies-mail"
        )
        XCTAssertEqual(
            LimeghostIconCatalog.resolvedIconID(iconID: "stickies-duo-nothing-like-this", legacyEmoji: "📁"),
            "folder",
            "a withdrawn identifier with no surviving drawing still falls back"
        )
    }

    /// The tint is the reason the Limeghost set names no colour. A licensed
    /// set carries its own, so the swatch row must not pretend otherwise.
    func testOnlyTheLimeghostSetIsTintable() {
        XCTAssertTrue(LimeghostIconStyle.limeghost.isTintable)
        XCTAssertFalse(LimeghostIconStyle.stickies.isTintable)
        XCTAssertFalse(LimeghostIconStyle.emojiOne.isTintable)

        XCTAssertNil(LimeghostIconStyle.limeghost.attribution)
        XCTAssertEqual(LimeghostIconStyle.stickies.attribution, "Stickies by Streamline, CC BY 4.0")
        ...
        for style in LimeghostIconStyle.allCases {
            let shapes = LimeghostIconCatalog.icons(style: style)
    ...
    func testEmojiSetShipsWholeAndInItsOwnBox() {
        let emoji = LimeghostIconCatalog.icons(style: .emojiOne)
        ...
        XCTAssertNil(
            LimeghostIconCatalog.icon(id: "emoji-flag-for-mexico"),
            "the one gradient drawing is skipped rather than approximated"
        )
        XCTAssertEqual(
            LimeghostIconStyle.emojiOne.attribution,
        ...
        for category in filled {
            XCTAssertFalse(LimeghostIconCatalog.icons(in: category, style: .emojiOne).isEmpty)
        }
    }
    ...
    func testEveryIconHasAUniqueIdentifierAndKeepsItsCategoryOrder() {
        let ids = LimeghostIconCatalog.all.map(\.id)
        XCTAssertEqual(Set(ids).count, ids.count, "identifiers are the stored value; duplicates would collide")
        for icon in LimeghostIconCatalog.all {
            XCTAssertFalse(icon.markup.isEmpty)
            XCTAssertEqual(LimeghostIconCatalog.icon(id: icon.id)?.id, icon.id)
        }

        let categoryOrder = LimeghostIconCatalog.icons(style: .limeghost).map(\.category)
        let expectedOrder = LimeghostIconCategory.allCases.flatMap { category in
            Array(repeating: category, count: LimeghostIconCatalog.icons(in: category).count)
        }
        XCTAssertEqual(categoryOrder, expectedOrder, "the catalog runs category by category, in picker order")

        XCTAssertNil(LimeghostIconCatalog.icon(id: "no-such-icon"))
        XCTAssertNotNil(LimeghostIconCatalog.icon(id: LimeghostIconCatalog.defaultIconID))
    }
    ...
    func testEveryShippedIconStaysInsideItsSixteenPointBox() {
        ...
        for icon in LimeghostIconCatalog.icons(style: .limeghost) {
    ...
    func testEveryStickiesIconStaysInsideTheBoxItDeclares() {
        ...
        XCTAssertLessThanOrEqual(
            worst.overshoot,
            LimeghostIconStyle.stickies.strokeWidth / 2,
    ...
    /// their own colours, which is exactly what the Limeghost set never does.
    func testStickiesArtworkCarriesItsOwnColours() {
        var seen = Set<String>()
        for icon in LimeghostIconCatalog.icons(style: .stickies) {
            guard let shapes = VectorPathParser.parse(icon.markup) else {
    ...
    func testTheFourTintsAreTheOnesTheArtworkWasDrawnAgainst() {
        XCTAssertEqual(LimeghostIconColor.allCases.map(\.rawValue), ["mint", "grey", "amber", "blue"])
        XCTAssertEqual(LimeghostIconColor.mint.hex, "66DB7D")
        XCTAssertEqual(LimeghostIconColor.grey.hex, "8A8A94")
        XCTAssertEqual(LimeghostIconColor.amber.hex, "E9B04C")
        XCTAssertEqual(LimeghostIconColor.blue.hex, "5CA0F2")

        let mint = LimeghostIconColor.mint.components
...
final class BookmarkFolderIconTests: XCTestCase {
    ...
    func testUnknownOrBlankIconIDFallsBackInsteadOfLeavingAHole() {
        ...
        XCTAssertEqual(blank.resolvedIconID, LimeghostIconCatalog.defaultIconID)

        XCTAssertEqual(
            LimeghostIconCatalog.resolvedIconID(iconID: "nothing-like-this", legacyEmoji: "🦄"),
            "folder",
    ...
    func testLegacyEmojiMapCoversEveryEmojiTheOldPickerOffered() {
        ...
        for (emoji, iconID) in zip(offered, expected) {
            XCTAssertEqual(LimeghostIconCatalog.iconID(forLegacyEmoji: emoji), iconID, "\(emoji) lost its meaning")
        }
        ...
        XCTAssertEqual(LimeghostIconCatalog.iconID(forLegacyEmoji: "🦄"), "folder", "anything unmapped is a folder")
        XCTAssertEqual(LimeghostIconCatalog.iconID(forLegacyEmoji: ""), "folder")
        XCTAssertEqual(
            LimeghostIconCatalog.iconID(forLegacyEmoji: "❤"),
            LimeghostIconCatalog.iconID(forLegacyEmoji: "❤️"),
    ...
    func testEveryLegacyEmojiPointsAtAnIconTheCatalogActuallyHas() {
        ...
        for emoji in emojiToCheck {
            let iconID = LimeghostIconCatalog.iconID(forLegacyEmoji: emoji)
            XCTAssertNotNil(LimeghostIconCatalog.icon(id: iconID), "\(emoji) maps to a missing icon: \(iconID)")
        }
    }
