...
/// Limeghost, so most of what is tested here is what happens when they are
...
final class BookmarkImportExportTests: XCTestCase {
    ...
    /// fields Limeghost ignores (`id`, `guid`, `meta_info`, `sync_metadata`),
    ...
    /// The round trip that matters most: what Limeghost writes, Limeghost
    ...
    /// with the bar *named* "toolbar", and Limeghost renames it to
    ...
    /// A root Limeghost does not recognize is never the bar, however it is
    ...
    func testNetscapeWithNoToolbarAttributeMarksNothing() throws {
        ...
        XCTAssertFalse(result.declaresBookmarksBar, "Limeghost never guesses the bar from a title")
    }
    ...
    /// Limeghost's exporter has always written `PERSONAL_TOOLBAR_FOLDER`
    /// on the bar; until now Limeghost could not read it back, so its own
    /// export lost the bar on the way in. This closes that loop.
    func testLimeghostExportRoundTripsItsOwnToolbarMarker() throws {
        ...
        XCTAssertTrue(result.declaresBookmarksBar, "Limeghost must be able to read its own export")
        XCTAssertEqual(result.roots.map(\.role), [.bookmarksBar])
    }
