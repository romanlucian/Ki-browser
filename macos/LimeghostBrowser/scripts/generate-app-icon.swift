...
private func loadMark(_ name: String) -> NSImage? {
    let repoRoot = URL(fileURLWithPath: #filePath)
        ...
        .deletingLastPathComponent()   // LimeghostBrowser
...
let fullMarkImage = loadMark("limeghost-mark-full.png")
let smallMarkImage = loadMark("limeghost-mark-small.png")
...
func renderIcon(pixels: Int) throws -> Data {
    guard let bitmap = NSBitmapImageRep(
        ...
        throw NSError(domain: "LimeghostIcon", code: 1)
    }
    ...
    if let artwork = markArtwork {
        ...
        guard let png = bitmap.representation(using: .png, properties: [:]) else {
            throw NSError(domain: "LimeghostIcon", code: 2)
        }
        return png
    }
...
for record in iconRecords {
    guard let png = renderedByPixelSize[record.pixels] else {
        throw NSError(domain: "LimeghostIcon", code: 3)
    }
