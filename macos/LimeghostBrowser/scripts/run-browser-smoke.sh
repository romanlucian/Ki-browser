...
# server, and the LIMEGHOST_SMOKE_BASE_URL that unlocks the live checks.
...
TEMP_DIR=$(mktemp -d /private/tmp/limeghost-browser-smoke.XXXXXX)
...
export LIMEGHOST_SMOKE_BASE_URL="http://127.0.0.1:$SMOKE_PORT/"
curl -fsS --retry 20 --retry-delay 0 --retry-connrefused "$LIMEGHOST_SMOKE_BASE_URL" >/dev/null
