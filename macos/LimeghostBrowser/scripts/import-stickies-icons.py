...
file is one icon in one of two forms — a plain one, and a `-duo` one drawn in
a single lime with a hard offset shadow. Only the plain form is imported: the
duo lime sits close enough to Limeghost's mint accent to read as a failed
match rather than a deliberate second colour, so the two greens fought each
other in the same bar. The `-duo` files in the source directory are skipped.

Markup is stored the way the Limeghost set is: verbatim, minus the `<svg>`
...
# Category order must match LimeghostIconCategory's declaration order, which
...
def main():
    ...
    for path in files:
        ...
        grouped.setdefault(CATEGORIES[name], []).append(("stickies-" + name, name, box, body))
    ...
    out = [
        ...
        "/// The set also ships a single-lime `duo` form, which is deliberately not",
        "/// imported: its green sits too close to the mint accent to read as a",
        "/// choice rather than a mismatch. Unlike the Limeghost set these carry",
        "/// their own colours, so a folder tint has nothing to act on; that is the",
        "/// artwork's nature, not a missing feature.",
        "///",
    ...
    for category in CATEGORY_ORDER:
        for icon_id, base, box, body in grouped.get(category, []):
            total += 1
            rows.append(
    ...
    destination = (
        ...
        / "Sources/LimeghostCore/StickiesIconCatalogData.swift"
    )
    destination.write_text("\n".join(out))
    print(f"wrote {total} icons to {destination} ({skipped} duo files skipped)")
