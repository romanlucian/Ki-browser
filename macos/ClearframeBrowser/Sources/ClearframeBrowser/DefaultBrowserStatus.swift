...
/// Whether this Mac opens links in Clearframe, and the one action that changes
/// it.
///
...
/// than the one you live in. Clearframe already declares `http` and `https` in
...
struct DefaultBrowserSettingsSection: View {
    ...
    var body: some View {
        Section("Default browser") {
            switch status.state {
            case .isDefault:
                Label("Links on this Mac open in Clearframe", systemImage: "checkmark.circle.fill")
            ...
            case .notDefault(let current):
                HStack {
                    ...
                    Spacer()
                    Button("Make Clearframe the default") { status.makeDefault() }
                }
            case .unknown:
                HStack {
                    Text("macOS did not report which browser opens links")
                    Spacer()
                    Button("Try to set Clearframe") { status.makeDefault() }
                }
            }
            ...
            if status.lastRequestFailed {
                ...
                Text("Clearframe asks macOS, and macOS asks you. Nothing changes without your confirmation, and you can change it back in System Settings → Desktop & Dock at any time.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
