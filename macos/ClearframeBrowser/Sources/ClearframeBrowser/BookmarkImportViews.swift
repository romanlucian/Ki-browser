...
struct BookmarkImportApplyResult: Equatable {
    ...
    /// Clearframe's own doing rather than something the person recognizes
...
struct BookmarkImportSheet: View {
    ...
    private func previewStage(_ info: PreviewInfo) -> some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if plan.isEmpty {
                Text("Every bookmark Clearframe found here — \(Self.countedLabel(plan.sourceBookmarkCount, "bookmark")) in all — is already saved. There's nothing new to import.")
                    .font(.callout)
                    .foregroundStyle(.secondary)
            } else {
    ...
    static func collisionWarning(_ titles: [String]) -> String {
        ...
        return "You already have \(verb) on your bar: \(list). Clearframe won't merge into \(titles.count == 1 ? "it" : "them") or rename anything, so you'll see both — yours untouched, and the imported one beside it."
    }
