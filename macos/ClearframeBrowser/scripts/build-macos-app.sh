...
ICONSET_DIR="$PACKAGE_DIR/.build/Clearframe.iconset"
ICON_FILE="$PACKAGE_DIR/.build/Clearframe.icns"
SIGNING_IDENTITY=${CLEARFRAME_SIGNING_IDENTITY:--}

mkdir -p "$MODULE_CACHE"
STAGING_ROOT=$(mktemp -d "$PACKAGE_DIR/.build/clearframe-app-stage.XXXXXX")
STAGED_APP="$STAGING_ROOT/Clearframe.app"
...
install -m 755 "$BUILD_DIR/ClearframeBrowser" "$CONTENTS_DIR/MacOS/ClearframeBrowser"
...
cp "$ICON_FILE" "$CONTENTS_DIR/Resources/Clearframe.icns"
touch "$STAGED_APP"

plutil -lint "$CONTENTS_DIR/Info.plist"
plutil -lint "$CONTENTS_DIR/Resources/PrivacyInfo.xcprivacy"
plutil -lint "$PACKAGE_DIR/App/Clearframe.entitlements"
if [[ "$SIGNING_IDENTITY" == "-" ]]; then
    codesign --force --options runtime \
        --entitlements "$PACKAGE_DIR/App/Clearframe.entitlements" \
        --sign - "$STAGED_APP"
else
...
PREVIOUS_APP="$STAGING_ROOT/Previous-Clearframe.app"
