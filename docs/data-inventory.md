...
**Status:** August 20, 2026. A complete record of what Limeghost stores, where it stores it, and what leaves the user's Mac. Read from the source, not from marketing copy.
...
**The short version.** Limeghost has no server, no account system, and no telemetry. Almost everything it stores stays in the user's own macOS profile. Four things leave the Mac, all of them either initiated by the user or unavoidable for browsing to work, and each is listed in section 2.
...
| Key | Contents |
|---|---|
| `limeghost.bookmarks.v1` | Saved bookmarks: title, address, creation date, folder |
| `limeghost.bookmarkFolders.v2` | Folder titles, icons, tints, hierarchy |
| `limeghost.history.v1` | Visit history: title, address, timestamp. **Capped at 500 entries** |
| `limeghost.workspace.v1` | Open tabs (address and title only), selection, tab groups |
| `limeghost.restoreTabs` · `limeghost.reloadRestoredTabs` · `limeghost.saveHistory` · `limeghost.showBookmarksBar` | Preference switches |
| `limeghost.searchEngine` | Chosen search provider |
| `limeghost.contentBlocking.enabled` · `.disabledHosts` | Tracker blocking state and per-site exceptions |
| `limeghost.onboarding.completed.v1` | First-run completion flag |
| `limeghost.showAssistantPanel` | Whether a tab opens with the assistant panel showing |
| `<key>.lastKnownGood` · `<key>.unreadable` | Corruption-recovery copies of the four record keys above |

### Files on disk

`~/Library/Application Support/Limeghost/Favicons/<host>.png` — site icons, downscaled to 64px, capped at 256 KB each.
`~/Library/Application Support/Limeghost/Favicons/redirects.json` — which hosts redirect to which, learned from visits already made, so a bookmark saved at a redirecting address can find its icon. Hostnames only, no addresses or page titles.

### Keychain

**Nothing.** Limeghost stores no credential of any kind. It once held a user-supplied OpenAI key; that feature was removed on August 30, 2026 and the Keychain item with it.
...
No other processor. No analytics vendor, no error-reporting service, no advertising network, no content delivery network of Limeghost's own.

Limeghost claims **no partnership or revenue arrangement** with any search provider or AI vendor.

## 6. Regulatory posture

Not legal advice, and no compliance assessment has been performed.

The structural position is unusually simple: for almost all of the data above, Limeghost never receives it. Bookmarks, history, tabs, and site icons exist only in the user's own macOS profile. There is no server, so there is no store to breach, no retention schedule to operate, and no export or deletion request that Limeghost could fulfil — because Limeghost holds nothing to export or delete. Deletion is fully in the user's hands through the controls in section 4.
