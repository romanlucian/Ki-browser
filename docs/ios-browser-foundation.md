...
Eighteen source files under `ios/Sources`, plus seven Swift files and one image compiled in by reference from the Mac target (below). What a person can do with it:
...
- **A bottom bar** — back, the address, the tab count and the page menu — placed along the bottom because that is where a thumb reaches and because the page's own top is worth leaving alone. It shows the host with `www.` trimmed, or "Search or enter a website" when there is nothing to show.
...
swift test    # 517 executed, 0 failures as of this document (skips vary 2-4
...
xcodebuild test -scheme Limeghost \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro'   # 73 executed, 0 failures
```
