...
Fourteen source files under `ios/Sources`, plus six Swift files and one image compiled in by reference from the Mac target (below). What a person can do with it:
...
- **A page menu** behind the bar's `•••`: Reader and Copy for AI as two large buttons, then Reload, Forward, New Tab, New Private Tab, Add Bookmark, Find in Page, Share and Request Desktop Site. It opens as tall as its rows rather than at a half-screen detent, because at half height the last rows opened below the fold. On the AI guide the page rows are greyed and the two new-tab rows still work. A row closes the sheet before it acts (`PageMenuPresentation`), because the system share sheet cannot be presented over it. The design is [docs/superpowers/specs/2026-09-12-ios-page-menu-design.md](superpowers/specs/2026-09-12-ios-page-menu-design.md).
...
**50 tests** run against the app on a Simulator. They cover the session platform, the workspace host, the bottom bar's model, the address sheet's submit, its refusal to submit an empty field, its completion and its refusal to complete a private tab, the switcher's private split, the start surface's three doors (including that opening a reference uncovers the page), whether the brand mark's artwork is in the phone's own bundle, whether the guide's header fits a phone's first screen at three iPhone widths, the page menu's model, actions and close-then-act rule, the find bar's outcome text, Reader's touch header at three widths, and Reader drawn over the page but never over the guide.
...
swift test    # 516 executed, 0 failures as of this document (skips vary 2-4
...
xcodebuild test -scheme Limeghost \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro'   # 50 executed, 0 failures
```
