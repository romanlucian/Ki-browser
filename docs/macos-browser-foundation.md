...
The primary prototype is a native SwiftUI application under `macos/LimeghostBrowser`. It launches as its own macOS process and window. `WKWebView` renders websites inside that window; it is not an extension hosted by Chrome, Brave, or Edge.

The current version-1 release stage includes:

- multiple independent tabs with a clear tab strip, `⌘T` new-tab, `⌘⇧N` private-tab, `⌘W` current-tab close, next/previous shortcuts, safe teardown, and popups routed into same-privacy-mode tabs; a `window.open()` popup adopts the web view WebKit hands over, so `window.opener` stays connected and a popup sign-in can report back to the page that started it, a popup opened with no address yet still gets a tab, and `target="_blank"` links open a credential-free HTTP/HTTPS tab without an opener; a ⌘-click or middle click on a link opens it in a tab behind the page, and ⇧⌘-click opens it in front, as every Mac browser reads those clicks (until September 22, 2026 both replaced the page being read; only a clicked link is moved, never a script's navigation or a submitted form);
...
- back, forward, home, reload, and stop from the toolbar, from the `⌘R`, `⌘.`, `⌘[`, and `⌘]` menu commands (each disabled when it cannot act), and from the two-finger trackpad back/forward gesture; a back or forward move onto a tab's own start-surface entry restores that start surface instead of failing as an unsupported link; real WebKit loading progress, HTTPS indication, and same-document URL/title synchronization per tab; reload after a failed load asks for the failed page again rather than reloading the page from before the failure, which WebKit still held; a page with no title of its own — plain text, an image, a bare document — is named by its address rather than "Loading…", and a title a page sets after it finished loading replaces the recorded one in history, once;
- links Limeghost does not navigate to never replace the page being read: `mailto:` and `tel:` go to the macOS app that owns them, and any other unsupported scheme is named in a dismissible notice above the page;
...
- a visible address-bar and Settings chooser for DuckDuckGo, Google, Bing, Brave Search, and Startpage, persisted only in local preferences; the query is percent-encoded so characters these engines read as query syntax — `+` above all — reach them as the words that were typed; an address typed without a scheme goes over HTTPS, except this machine and the local network — `localhost`, a `.local` name, an IP address — which go over HTTP, because a development server or a router has no certificate to offer and the HTTPS attempt only failed;
...
- a full-page bookmarks home (⌘⌥B or the bookmarks bar) with folder cards showing rolled-up bookmark/subfolder counts, search across bookmarks and folder titles, and drill-down navigation; a separate full-page history surface (⌘Y, or History ▸ Show Full History) grouping stored visits by day with its own search and Clear History; the toolbar's books button keeps a quick bookmark popover, which is the drill-down organizer rather than a smaller copy of the home;
- local bookmarks organized into titled, emoji-labeled folders and nested subfolders, including safe legacy migration to Unfiled, folder create/rename/delete, bookmark moves, and searchable local history with per-entry removal and a confirmed Clear History on the history page; turning off “Save browsing history” stops Limeghost recording new visits and leaves the visits already stored in place; that switch and “Load every restored tab at start” belong to the profile whose Settings they were changed in (until September 22, 2026 Settings wrote them where only the original profile reads, so in any other profile neither did anything);
...
- source-language extraction of a page's rendered reading text with interface noise removed, a read-time estimate, explained risk signals, Copy for AI, and Reader (`⇧⌘R`), which draws that extracted text on its own so the complete payload can be read before any of it is copied;
- a one-source save / second-source comparison flow;
- API-key storage in macOS Keychain;
- non-persistent private-tab website storage, no private history/restoration, a user-confirmed reset of the profile's browser data, external HTTP/HTTPS event handling, visible JavaScript/media-permission prompts — from a site's second JavaScript dialog on, with “Don't allow more dialogs from this site”, because a page asking in a loop otherwise holds the window — and a renderer-termination error state;
...
DuckDuckGo is the changeable initial search default. Submitted search text goes to the selected provider’s ordinary HTTPS results URL; direct website addresses do not. Limeghost requests no search suggestions while the user types and claims no partnership, default-search contract, or revenue-share agreement.
...
`LimeghostCore` is intentionally UI- and WebKit-independent:
...
`LimeghostBrowser` is macOS-specific:
...
- the Halo chrome's design tokens (surfaces, text tiers, accent, hairlines, radii, meta type), centralized in `LimeghostTheme.swift` rather than restated per view.
...
Open `dist/Limeghost.app` from Finder at the repository root. The script generates the app icon, embeds `PrivacyInfo.xcprivacy`, validates bundle metadata, enables the hardened runtime with narrow camera/microphone entitlements, and ad hoc signs/verifies the bundle for local execution. `LIMEGHOST_SIGNING_IDENTITY` can select an available signing identity, but notarization is a separate credentialed release step. The checked-in result is not Developer ID signed or notarized.
...
- **Provider-page performance varies.** Limeghost uses WebKit's default persistent website data store and does not proxy, prefetch, or rewrite third-party media. Media-heavy pages such as ByteDance Seed may load images or video incrementally as the user scrolls. Compare the exact URL in Safari on the same connection, once cold and once after caching; a repeatable Limeghost-only slowdown needs a separate WebKit/network trace.
- **Basic tabs, not Chrome-scale tab management.** This release was single-window and had no tab reordering, pinned tabs, tab groups, multiple profiles, separate private windows, or cross-device sync. Tab reordering, pinned tabs, tab groups, multiple windows, and dragging a tab into its own window landed later, in August 2026, and multiple profiles and private windows on August 21, 2026; sync remains absent. Deleting a profile closes every window it has open, and its data — preferences, site icons, profile picture and WebKit website data — is erased by a record that survives a quit: WebKit refuses to remove a store still in use, so what could not go at once is finished at the next launch. Private tabs are clearly marked, use an ephemeral WebKit data store, and avoid local history/restoration, but do not provide network anonymity. Restoration reloads the current URL; it does not preserve back/forward stacks, form state, or page content.
...
- **Default-browser registration is only a prerequisite.** The app declares HTTP/HTTPS handling and safely opens incoming web URLs, but Limeghost does not silently change the user's default browser. Finder/System Settings registration and default selection still need release QA after a Developer ID-signed installation.
- **Security claims are narrow.** WebKit provides a modern rendering process, but Limeghost has not completed an independent browser security review. Its risk scanner reads only visible text and page-level facts.
...
3. Add a permission center and certificate details beyond the connection state the site panel already reports.
