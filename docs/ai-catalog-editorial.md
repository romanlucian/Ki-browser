...
Limeghost’s AI home is a small local guide for ordinary users. Catalog records live in `macos/LimeghostBrowser/Sources/LimeghostCore/AIToolCatalogData.swift`, compile into the app, and change only through a reviewed app update. There is no catalog server, analytics feed, affiliate ordering, or automatic remote fetch in this phase.

## What the labels mean

- **Best Overall** means the editor considers the tool the strongest general starting point for one named task within Limeghost’s small catalog.
...
These rationales describe intended product focus. Limeghost has not benchmarked the providers against one another, and provider-controlled features and terms can change after the checked date.
...
The catalog names other companies' products and links to their official sites. That is referential use: a directory cannot describe a tool without naming it, so the names appear as plain text in the app's own typeface, with no logo, no brand styling, and no artwork of any provider bundled with Limeghost. A tool's own icon appears only if the reader has opened it and the browser captured it during that visit, the same way it captures any site's icon. The AI home states in the app that Limeghost is independent, is not affiliated with or endorsed by any listed provider, and that the names are their owners' trademarks.
...
- a first-party Limeghost HTTPS endpoint, not arbitrary raw GitHub URLs;
