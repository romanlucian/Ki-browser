...
- Address-bar completion reads the local history and bookmark records already described above and nothing else. It makes no network request, contacts no suggestion service, and proposes only addresses this profile has already visited or saved; a prefix that matches nothing completes to nothing rather than guessing a host. It is disabled in private tabs: nothing is written there either way, but reading a saved history back onto the screen would work against what a private tab is for.
