...
| Trading identity | **Zincoo** — zincoo.com — credited in the app as "Clearframe by Zincoo" |
| Product | Clearframe, a standalone macOS browser |
...
**Clearframe has no third-party code dependencies.** Verified: `macos/ClearframeBrowser/Package.swift` declares only internal target dependencies (`ClearframeCore`, `ClearframeBrowser`), and `package.json` declares no runtime or development packages. There is no `node_modules`, no vendored library, no package manager pulling external source into a build.
...
- **The CEF scaffold** at `chromium/cef-spike` — an isolated architecture validation containing Clearframe's own Objective-C header and CMake configuration. **No Chromium or CEF source or binary is committed to this repository.** It is not linked into the shipping app. If a future Chromium migration proceeds, CEF's own licence terms (BSD-style, plus Chromium's own components) become relevant and must be reviewed then.
...
|---|---|---|---|
| **Clearframe** | 104 | Original work, authored for this project | None. Owned outright. The only tintable set — drawn in `currentColor`. |
...
**The name Clearframe is not cleared.** This is the most significant unresolved IP item.
...
- `clearframe.com`, `clearframe.app`, and `getclearframe.com` are all already registered to other parties.

The current plan is to distribute from **`zincoo.com/clearframe`** — a domain the founder controls — following the pattern Firefox and Orion use, which also places an executable download on an established domain rather than a new one.
...
- Architecture and licensing decisions: the AGPL choice, the `ClearframeCore` boundary, the shared analysis contract, the favicon privacy policy.
...
[LICENSE](../LICENSE) · [privacy-and-safety.md](privacy-and-safety.md) · [data-inventory.md](data-inventory.md) · [product-foundation.md](product-foundation.md) · [macos-browser-foundation.md](macos-browser-foundation.md) · [clearframe-strategy.md](clearframe-strategy.md)
