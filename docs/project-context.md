...
- A unified single-row dark chrome with inline traffic lights in the tab strip, one toolbar/address-pill row, and deterministic per-site identity-color dots derived locally from each host—no favicon fetching.
