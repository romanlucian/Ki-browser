...
- A visible-by-default native bookmarks bar below navigation with top-level folders that always render one emoji-plus-name label (long names truncate rather than becoming icon-only), Unfiled links, recursive folder menus, horizontal scrolling, fixed overflow access, and a locally persisted show/hide control. Standard macOS secondary-click/Control-click menus add the current page, create root folders or nested subfolders, and open the organizer; the More and Page menus provide accessible alternatives, including ⌘⌥B for the organizer.
