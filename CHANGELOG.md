...
- Closing a window left every web view in it alive. A page that was playing went on playing — audible, with nothing on screen left to stop it, which is exactly how it was found. Nothing tore a window's tabs down: closing a *tab* called `teardown`, but closing the *window* only dropped a dictionary entry. A window now tears down its tabs and its assistant panel when it closes, hooked to the window's own close rather than to a SwiftUI `onDisappear`, which reports that a view went away rather than that a window did.
- `teardown` itself never stopped media either, so closing a *tab* with a video playing leaked audio the same way. `stopLoading` ends the network fetch and does nothing to a `<video>` that has already buffered. It now pauses playback, leaves full screen and picture-in-picture, and finally replaces the document — pausing alone is not enough, because the page's own script can resume it.
- Verified on the running app: with a video playing in a private window, closing that window dropped its web content process from playing to 0.1 per cent CPU. The new test fails, naming each surviving web view, when the window-close teardown is removed.
