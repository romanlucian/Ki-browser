...
- **A second gate, found while writing the first one down.** The phone's address sheet completes a private tab from the ordinary profile's bookmarks and history. The Mac refuses to: `BrowserView.addressSuggestions` opens with `guard !session.isPrivate`. The sheet's own comment records that there was no way to open a private tab when it was written and that the guard had to arrive with whichever task added one — the tab switcher then added private tabs and the guard did not come with it. Nothing private leaks into the store, but a private tab shows what was browsed outside it, which is the line the product draws deliberately. Named as a blocking item with its fix rather than patched into the commit that discovered it, so the fix arrives with its own test.
...
- CI's existing `ios-simulator` job gained a second step that builds and tests the app on the simulator it already discovers. Not a second job, and `macos-browser` untouched. The project carries no checked-in scheme; `xcodebuild` autocreates one, which was verified from a tracked-files-only copy of the repository — 22 tests, `** TEST SUCCEEDED **` — rather than assumed, because a missing scheme is a failure with nothing in the diff to explain it. **The job itself has still never run**, since `ci.yml` triggers only on push to `main` and on pull requests.
...
- 492 Mac tests, 0 failures; 22 iOS tests, 0 failures. There is no assistant, no Reader, no Copy for AI and no bookmark import in the phone app, and no document says otherwise. **No observed-user session has been run for this project on either platform.**
