...
The shared layer runs **256 tests** on both destinations, and the app's own suite runs **28**. Take the 256 from `swift test --list-tests` (236 `LimeghostCoreTests` + 20 `LimeghostSharedTests`; the other 248 of the Mac's 504 are `BrowserBehaviorTests`) and **never from a run's own log**: xcodebuild runs that scheme across parallel workers, which suppresses the `Executed N tests` summary entirely and interleaves the per-case lines, and an interleaved line can be cut mid-name — that is how a correct 249 was once "corrected" to 248. The app's 28 does come from its run, which is not parallelised and prints the summary; read it with `grep -E "^\*\* TEST|Executed [0-9]+ tests"` rather than `tail`, for the same reason the counts above are grepped.
