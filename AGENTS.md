...
- `macos/LimeghostBrowser/Tests/LimeghostCoreTests/Fixtures/local-analysis-contract.json` — page structure, risk, reading time, sentence segmentation, and the three interface-noise filters, with expected output; 46 cases across 8 keys. `riskWindowCases` describe pages too long to write out — a text repeated a number of times, then a suffix — and both harnesses build them the same way;
...
The shared layer runs **322 tests** on both destinations, and the app's own suite runs **92**. Take the 322 from `swift test --list-tests` (247 `LimeghostCoreTests` + 75 `LimeghostSharedTests`; the other 268 of the Mac's 590 are `BrowserBehaviorTests`) and **never from a run's own log**: xcodebuild runs that scheme across parallel workers, which suppresses the `Executed N tests` summary entirely and interleaves the per-case lines, and an interleaved line can be cut mid-name — that is how a correct 249 was once "corrected" to 248. The app's 92 does come from its run, which is not parallelised and prints the summary; read it with `grep -E "^\*\* TEST|Executed [0-9]+ tests"` rather than `tail`, for the same reason the counts above are grepped.

Several shared tests load real pages through WebKit, and **the first page in a fresh Simulator process can take over a minute**: measured on September 22, 2026, the first `SessionPageTests` case waited 64 seconds for WebKit to start and every later one about one second. Their waits are long on purpose — a met condition returns at once — so do not "tighten" them to a few seconds; CI's cold runners are exactly where they would then fail.

**On CI's iOS simulator, one page's web process can stall for minutes while every other test process on the same simulator keeps loading pages normally.** Measured in the shared suite's bare xctest host: 750 s in `testASchemeTheBrowserDoesNotRenderGoesToThePlatform` on `main` (September 13, 2026), then 956 s, 157 s and at least 155 s in three different tests across pull request #4's first two runs (September 23), on an iOS 26.2 simulator. The same tests take about a second locally and in the macOS job. The result bundle of the second run showed WebKit had not even committed a new tab's blank document after 90 s. So the `ios-simulator` job, and only that job, retries a failed test twice (`-retry-tests-on-failure -test-iterations 3`), and uploads its result bundles when it fails. Do not lengthen the waits to cover the stalls instead: at 956 s, a real regression in page loading would then take hours to fail on the Mac. `SessionPageTests.require` ends a test at the first wait that runs out, because a page handed to WebKit before the tab's own document finished is never registered by the tab, and every later assertion would fail for that reason alone.

The `ios-simulator` CI job runs the shared-layer scheme, and now the app scheme too, against a Simulator it discovers at runtime on GitHub's macos-15 image. Both commands pass here against a real iPhone 17 Pro simulator, and **the job itself first ran on September 13, 2026, on pull request #1** — `ci.yml` triggers only on push to `main` and on `pull_request`, so a branch push runs nothing, and until then it had never executed. Its first run failed to load the shared test bundle (`libswiftWebKit.dylib`) on the device its picker happened to choose; the picker now takes the newest iOS runtime and prints its choice, and the second run passed. And a passing Simulator run is not a passing *device* run: the Simulator shares the Mac's own libraries and file system, so it cannot show a sandbox, entitlement, or memory-pressure difference. The app was first installed on a real iPhone on September 10, 2026, by cable and under free provisioning; [docs/ios-browser-foundation.md](docs/ios-browser-foundation.md) records what that took.
